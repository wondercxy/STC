...
public class STC {
	...
	public int[][] hour;
	public int[][] h;
	public int[][] hourTopicCount;
	public int[][] topicHourItemCount;
	public int[] hourTopicCountSum;
	public int[] topicHourItemCountSum;
	double[][] topicHourItemDistribution;
	double[][] hourTopicDistribution;
	...
	public HashMap<Integer, ArrayList<String>> user_content;
	...
	public void RegionLocationMaker() throws Exception {
		...
		while (line != null) {
			...
			if (user_content.containsKey(u_id)) {
				...
				ArrayList<String> list = user_content.get(u_id);
				for (String s : cs) {
					list.add(s);
				}
				user_content.put(u_id, list);// 用户-类别
			} else {
	...
	public void InitialState() throws Exception {
		userTopicCount = new int[UserNumber][TopicNumber];// 用户：m文档中的k主题出现的次数
		topicItemCount = new int[TopicNumber][topic_dictionary.size()];// 用户：m文档中k主题出现的单词数
		userTopicCountSum = new int[UserNumber];// m文档中主题总数
		topicItemCountSum = new int[TopicNumber];// k主题对应的总词数

		topicItemDistribution = new double[TopicNumber][topic_dictionary.size()];
		userTopicDistribution = new double[UserNumber][TopicNumber];

		hourTopicCount = new int[HourNumber][TopicNumber];// 用户：m文档中的k主题出现的次数
		topicHourItemCount = new int[TopicNumber][topic_dictionary.size()];// 用户：m文档中k主题出现的单词数
		hourTopicCountSum = new int[HourNumber];// m文档中主题总数
		topicHourItemCountSum = new int[TopicNumber];// k主题对应的总词数

		topicHourItemDistribution = new double[TopicNumber][topic_dictionary.size()];
		hourTopicDistribution = new double[HourNumber][TopicNumber];
		...
		for (int m = 0; m < UserNumber; m++) {
			...
			if (user_content.containsKey(uid)) {
				...
				user[m] = new int[N];
				for (int n = 0; n < N; n++) {
					user[m][n] = docWords[n];// 得到文档-单词矩阵
				}
			}
		}

		z = new int[UserNumber][];
		for (int m = 0; m < UserNumber; m++) {
			int uid = userList.get(m);
			if (user_content.containsKey(uid)) {
				int N = user_content.get(uid).size();// 获取文档中不同单词的个数
				// System.out.println(N);
				z[m] = new int[N];
				for (int n = 0; n < N; n++) {
					int initTopic = (int) (Math.random() * TopicNumber);
					z[m][n] = initTopic;// 随机分配一个主题给单词
					// number of words in doc m assigned to topic initTopic add 1
					userTopicCount[m][initTopic]++;// 累加随机分配主题的单词个数
					// number of terms doc[m][n] assigned to topic initTopic add 1
					// System.out.println(initTopic + " " + m + " " + n);
					topicItemCount[initTopic][user[m][n]]++;// 累加分配给主题K文档m中的单词数
					// total number of words assigned to topic initTopic add 1
					topicItemCountSum[initTopic]++;// 累加分配给主题的单词总数
				}
				// total number of words in document m is N
				userTopicCountSum[m] = N;// m文档中主题总数
			}
		}

		hour = new int[HourNumber][];
		for (int m = 0; m < HourNumber; m++) {
			int N = hourcontent.get(m).size();
			int[] docWords = new int[N];// 记录单词索引的位置
			for (int i = 0; i < N; i++) {
				String word = hourcontent.get(m).get(i);
				// System.out.println(word);
				if (!hourtermToIndexMap.containsKey(word)) {
					int newIndex = hourtermToIndexMap.size();
					hourtermToIndexMap.put(word, newIndex);
					indexToHourTermMap.add(word);
					hourtermCountMap.put(word, new Integer(1));
					docWords[i] = newIndex;
				} else {
					docWords[i] = hourtermToIndexMap.get(word);
					hourtermCountMap.put(word, hourtermCountMap.get(word) + 1);// 记录单词出现的次数
				}
			}
			hour[m] = new int[N];
			for (int n = 0; n < N; n++) {
				hour[m][n] = docWords[n];// 得到文档-单词矩阵
			}
		}

		h = new int[HourNumber][];
		for (int m = 0; m < HourNumber; m++) {
			int N = hourcontent.get(m).size();// 获取文档中不同单词的个数
			h[m] = new int[N];
			for (int n = 0; n < N; n++) {
				int initTopic = (int) (Math.random() * TopicNumber);
				h[m][n] = initTopic;// 随机分配一个主题给单词
				hourTopicCount[m][initTopic]++;// 累加随机分配主题的单词个数
				topicHourItemCount[initTopic][hour[m][n]]++;// 累加分配给主题K文档m中的单词数
				topicHourItemCountSum[initTopic]++;// 累加分配给主题的单词总数
			}
			hourTopicCountSum[m] = N;// m文档中主题总数
		}
	...
	public void updateEstimatedParameters() {
		...
		for (int m = 0; m < UserNumber; m++) {
			for (int k = 0; k < TopicNumber; k++) {
				userTopicDistribution[m][k] = (userTopicCount[m][k] + alpha)
						/ (userTopicCountSum[m] + TopicNumber * alpha);
			}
		}
	}

	public void updateEstimatedParametersHour() {
		for (int k = 0; k < TopicNumber; k++) {
			for (int t = 0; t < topic_dictionary.size(); t++) {
				topicHourItemDistribution[k][t] = (topicHourItemCount[k][t] + 0.1)
						/ (topicHourItemCountSum[k] + topic_dictionary.size() * 0.1);
			}
		}

		for (int m = 0; m < HourNumber; m++) {
			for (int k = 0; k < TopicNumber; k++) {
				hourTopicDistribution[m][k] = (hourTopicCount[m][k] + alpha)
						/ (hourTopicCountSum[m] + TopicNumber * alpha);
			}
		}
	}
	...
	public int sampleTopicZ(int m, int n) {
		...
		int oldTopic = z[m][n];
		userTopicCount[m][oldTopic]--;
		topicItemCount[oldTopic][user[m][n]]--;
		userTopicCountSum[m]--;
		topicItemCountSum[oldTopic]--;

		double[] p = new double[TopicNumber];
		for (int k = 0; k < TopicNumber; k++) {
			p[k] = (topicItemCount[k][user[m][n]] + 0.1) / (topicItemCountSum[k] + topic_dictionary.size() * 0.1)
					* (userTopicCount[m][k] + alpha) / (userTopicCountSum[m] + TopicNumber * alpha);
		}
		...
		userTopicCount[m][newTopic]++;
		topicItemCount[newTopic][user[m][n]]++;
		userTopicCountSum[m]++;
	...
	public int sampleTopicH(int m, int n) {

		int oldTopic = h[m][n];
		hourTopicCount[m][oldTopic]--;
		topicHourItemCount[oldTopic][hour[m][n]]--;
		hourTopicCountSum[m]--;
		topicHourItemCountSum[oldTopic]--;

		double[] p = new double[TopicNumber];
		for (int k = 0; k < TopicNumber; k++) {
			p[k] = (topicHourItemCount[k][hour[m][n]] + 0.1)
					/ (topicHourItemCountSum[k] + topic_dictionary.size() * 0.1) * (hourTopicCount[m][k] + alpha)
					/ (hourTopicCountSum[m] + TopicNumber * alpha);
		}
		...
		hourTopicCount[m][newTopic]++;
		topicHourItemCount[newTopic][hour[m][n]]++;
		hourTopicCountSum[m]++;
		topicHourItemCountSum[newTopic]++;
	...
	public void saveIteratedModel(int iters) throws IOException {
		...
		BufferedWriter writer = new BufferedWriter(new FileWriter(resPath + modelName + ".topicItem"));
		...
		writer = new BufferedWriter(new FileWriter(resPath + modelName + ".userTopic"));
		...
		writer = new BufferedWriter(new FileWriter(resPath + modelName + ".topicHourItem"));
		for (int i = 0; i < TopicNumber; i++) {
			for (int j = 0; j < topic_dictionary.size(); j++) {
				writer.write(topicHourItemDistribution[i][j] + "\t");
			}
			writer.write("\n");
		}
		writer.close();

		writer = new BufferedWriter(new FileWriter(resPath + modelName + ".hourTopic"));
		for (int i = 0; i < HourNumber; i++) {
			for (int j = 0; j < TopicNumber; j++) {
				writer.write(hourTopicDistribution[i][j] + "\n");
			}
			writer.write("\n");
		}
		writer.close();
